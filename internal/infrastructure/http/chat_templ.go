...
func ChatMessages(msgs []chat.Message, currentUserID string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 23, "<div data-init=\"el.scrollIntoView()\"></div></div><div class=\"border-t border-slate-800 px-4 py-3 flex gap-2 items-end\"><textarea data-bind:chatmsg placeholder=\"Message…\" rows=\"1\" data-on:keydown=\"evt.key==='Enter' && !evt.shiftKey && !evt.preventDefault() && @post('/api/chat/send', {openWhenHidden: false}) && ($chatmsg='')\" class=\"flex-1 bg-slate-800 text-slate-100 border border-slate-700 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 placeholder-slate-600\"></textarea> <button data-on:click=\"@post('/api/chat/send', {openWhenHidden: false}) && ($chatmsg='')\" class=\"bg-cyan-600 hover:bg-cyan-700 text-white p-2 rounded-lg transition-colors flex-shrink-0\" title=\"Send\"><svg xmlns=\"http://www.w3.org/2000/svg\" class=\"w-4 h-4\" fill=\"none\" viewBox=\"0 0 24 24\" stroke-width=\"1.5\" stroke=\"currentColor\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" d=\"M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5\"></path></svg></button></div></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
